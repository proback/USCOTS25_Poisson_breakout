...
- Identify when an overdispersed model is necessary using graphical evidence, residual deviance and goodness of fit test. 

- Fit a quasipoisson model in R.

- Identify the overdispersion paramter from quasipoisson model output. 
...
here::i_am("2-Poisson-F24/01key_poisson_intro_elephants.Rmd")
...
elephant |>
  mutate(agegroup = cut_number(age, 10)) |>
  group_by(agegroup) |>
...
**The residual difference (51.09) is slightly larger than the df (39).**
...
```{r}
quasi.elephant <- glm(matings ~ age, data = elephant, family = quasipoisson)
summary(quasi.elephant)
```
...
In the poisson model, dispersion is assumed to be 1. In the quasipoisson, we estimate the overdispersion parameter to be 1.157. 
...
The AIC is NA for the quasipoisson model.
...
*Not really, this seems pretty good*

5. Conduct a goodness of fit test for the Poisson regression model. 

```{r}
1 - pchisq(pois.elephant$deviance, pois.elephant$df.residual)
```

$H0:$ Model fits well (no overdispersion), $HA$: Lack of fit (Possibly due to overdispersion, or for other reasons!)

*p-value is 0.09, so we fail to reject H0. There is not evidnce of overdispersion.*
...
**As the overdispersion is only estimated to be 1.15, this is not a lot of overdispersion! The overdispersed model is slightly more conservative, and so may be preferred even if the amount of overdispersion is small. (In this case, we see the effect of age is obviously statistically significant, whether or not overdispersion is accounted for).**

**The downside of choosing the quasi-Poisson model is that we do not get an AIC to use for non-nested comparisons.**
...
```{r}
wwc_games <- read_csv(here("2-Poisson-F24/Data/wwc_games.csv"))
...
*A Poisson regression model seems appropriate (skewed, count response). The plot shows a potential log-linear relationship... though maybe need a quadratic term? There is one big outlier, a game with 13 goals for one team!.*

2. Create a graph of mean vs variance with a blue and black line, similar to the one for elephants in the example code above. What does this suggest to use about the appropriateness of a Poisson regression model?

```{r}
wwc_games |> 
  mutate(spigroup = cut_interval(spi, 6)) |>
  group_by(spigroup) |>
  summarize(meanscore = mean(score), varscore = var(score), n = n()) |>
  ggplot(aes(x = meanscore, y = varscore)) +
  geom_point() + 
  geom_smooth(method = 'lm', se = FALSE) + 
  geom_abline(slope = 1, intercept = 0)
```
...
```{r}
wwc.pois <- glm(score ~ spi, data = wwc_games, family = poisson)
summary(wwc.pois)
exp(coef(wwc.pois))
exp(confint(wwc.pois))
```

*We are 95% confident that each additional SPI point is associated with an additional 1.4 to 4.0% higher number of goals in a game. *


4. Examine the summary output for this model. Does there appear to be evidence of overdispersion (or other lack of fit)?

*Some yes, the residual deviance (152.96) is somewhat greater than the residual degrees of freedom.*
...
```{r}
1 - pchisq(wwc.pois$dev, wwc.pois$df.residual)
```

*THere is significant evidence of lack of fit (p-value = 0.0008).*
...
```{r}
soccer.pred <- data.frame(fitted = predict(wwc.pois),
                            resid = resid(wwc.pois))

ggplot(soccer.pred, aes(x = fitted, y = resid)) + 
  geom_point() +
  geom_smooth(se = FALSE, color = "red", span = 2) + 
  geom_abline(intercept = 0, slope = 0)
```

*One large outlier (this was the game with 13 goals!) and possibly a quadratic relationship. 

7. What are some suggestions for further modeling?? Try a few possibilities.


```{r}
wwc.pois2 <- glm(score ~ spi + I(spi^2), data = wwc_games, family = poisson)
summary(wwc.pois2)

wwc.qpois2 <- glm(score ~ spi + I(spi^2), data = wwc_games, family = quasipoisson)
summary(wwc.qpois2)

soccer.pred2 <- data.frame(fitted = predict(wwc.pois2),
                            resid = resid(wwc.pois2))

ggplot(soccer.pred2, aes(x = fitted, y = resid)) + 
  geom_point() +
  geom_smooth(se = FALSE, color = "red", span = 2) + 
  geom_abline(intercept = 0, slope = 0)
```

Age squared is significant, and it seems to help a little with curvature of deviance residuals (Note: These never seem to become perfectly flat for Poisson...) and there is still the big outlier. The quadratic term is also significant after accounting for overdispersion (p = 0.049)


8. Give at least one reason why it may not be appropriate to assume the observations are independent in this dataset.

*the same teams play in multiple different matches*
...
The `glm.nb` function works a lot like  `glm`, but is in the `MASS` package. Because MASS has several conflicts with tidyverse, I usually use `MASS::glm.nb` rather than loading the entire package with `library()`. 


Can you modify the code below to fit one of your models from 7 but with a negative binomial rather than poisson? 

```{r}
?MASS::glm.nb

wwc.nb <- MASS::glm.nb(score ~ spi, data = wwc_games)

summary(wwc.nb)

wwc.nb2 <- MASS::glm.nb(score ~ spi + I(spi^2), data = wwc_games)

summary(wwc.nb2)

soccer.prednb2 <- data.frame(fitted = predict(wwc.nb2),
                            resid = resid(wwc.nb2))

ggplot(soccer.prednb2, aes(x = fitted, y = resid)) + 
  geom_point() +
  geom_smooth(se = FALSE, color = "red", span = 2) + 
  geom_abline(intercept = 0, slope = 0)
```
