...
ggplot(elephant, aes(x = age, y = matings)) + 
  ...
  labs(title = "Poisson regression model of matings",
       subtitle = "log(lambda) = -1.58 + 0.069age")
```
...
*This plot helps us check that there is a log-linear relationship of $\lambda$ (E(Y)) and X. It looks good!*
...
*This table helps us check the condition that mean = variance. Though the mean is not exactly equal to variance, the variance is increasing as the mean increases, so that is a good sign.*

Note: We can also plot the mean vs variance, in addition to looking at the table; try a few different values in `cut_number()`.
...
elephant |>
  ...
  labs(title = "Black line indicates mean = variance")
```
